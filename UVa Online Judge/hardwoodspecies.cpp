...
template <typename Clave, typename Valor, typename Comparador>
Valor & TreeMap<Clave, Valor, Comparador>::operator[](Clave const& clave) {
	return corchete(clave, raiz);
}
...
template <typename Clave, typename Valor, typename Comparador>
Valor & TreeMap<Clave, Valor, Comparador>::corchete(Clave const& c, Link & a) {
	if (a == nullptr) {
		a = new TreeNode(ClaveValor(c));
		return a->cv.valor;
	}
	else if (this->menor(c, a->cv.clave)) {
		Valor & v = corchete(c, a->iz);
		this->reequilibraDer(a);
		return v;
	}
	else if (this->menor(a->cv.clave, c)) {
...
void mostrarArbol(TreeMap<string, double> &arbol, const int &contador, const int &i, const int &n) {
	TreeMap<string, double>::Iterator it = arbol.begin();
	TreeMap<string, double>::Iterator itFin = arbol.end();

	while (it != itFin) {
		TreeMap<string, double>::ClaveValor c = *it;
		cout << c.clave << " " << fixed << setprecision(4) << (c.valor / contador) * 100 << '\n';
		it++;
	}
...
void resuelve(const int &i, const int &n) {
	TreeMap<string, double> arbol;
	string s;
	int contador = 0;

	getline(cin, s);
	while (s != "") {
		if (arbol.contains(s)) arbol.insert(s, arbol.at(s) + 1);
		else arbol.insert(s, 1);
		contador++;
		getline(cin, s);
	}

	mostrarArbol(arbol, contador, i, n);
