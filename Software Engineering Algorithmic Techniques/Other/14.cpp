//
//  14.cpp
//  14
//
//  Created by Daniel García Molero on 17/11/16.
//  Copyright © 2016 Daniel García Molero. All rights reserved.
//

//Grupo TAIS14
//Juan Alberto Camino Saez
//Daniel Garcia Molero

#include <iostream>
#include <vector>
#include <fstream>
#include <string>

#include "Matriz.h"

using namespace std;

class ConjuntosDisjuntos {
public:
    
    // crea una estructura de particiÛn con los elementos 0..N-1, cada uno en un conjunto
    ConjuntosDisjuntos(size_t N) : size(N), _count(size), id(size, -1), max(0){}
    
    // devuelve el n˙mero de conjuntos disjuntos
    size_t count() const { return _count; }
    
    //  devuelve el identificador del conjunto que contiene a p
    size_t buscar(size_t p) const {
        if (id.at(p) < 0) // es una raÌz
            return p;
        else
            return id[p] = int(buscar(id[p]));
    }
    
    size_t maximo() {
        return max;
    }
    
    // unir los conjuntos que contengan a p y q
    void unir(size_t p, size_t q) {
        size_t tam;
        auto i = buscar(p);
        auto j = buscar(q);
        if (i == j) {
            if (0 - id[i] > max) {
                max = 0 - id[i];
            }
            return;
        }
        if (id[i] < id[j]) { // i es la raÌz del mayor ·rbol
            id[i] += id[j]; id[j] = int(i);
            tam = id[i];
        }
        else {
            id[j] += id[i]; id[i] = int(j);
            tam = id[j];
        }
        _count--;
        
        if (0 - tam > max) {
            max = 0 - tam;
        }
    }
    
    // muestra el array por la salida o
    void print(std::ostream& o = std::cout) const {
        o << _count << ": ";
        for (auto i = 0; i < size; ++i)
            o << id[i] << " ";
    }
    
protected:
    size_t size;             // los elementos son 0 .. size-1
    size_t _count;           // n˙mero de conjuntos disjuntos
    size_t max;
    mutable std::vector<int> id;  // si id[i]>=0 es el padre de i, si no es el tamaÒo del conjunto de i
    mutable std::vector<int> tamanio;
    
};

inline std::ostream& operator<<(std::ostream & o, ConjuntosDisjuntos const& a) {
    a.print(o);
    return o;
}

void comprobarAlrededorInicial(const Matriz<size_t> &mapa, ConjuntosDisjuntos &c, int i, int j, int x, int y) {
    for (int k = i - 1; k <= i; ++k) {
        for (int l = j - 1; l <= j + 1; ++l) {
            if((k == i - 1 && l == j - 1) || (k == i + 1 && l == j - 1) || ((k == i + 1 && l == j + 1)) || ((k == i - 1 && l == j + 1))) continue;
            if (k >= 0 && k < x && l >= 0 && l < y) {
                if (mapa[k][l] == 1) {
                    c.unir(i * y + j, k * y + l);
                }
            }
            if (k == i && l >= j) {
                return;
            }
        }
    }
}

ConjuntosDisjuntos unirInicial(int x, int y, const Matriz<size_t> &mapa) {
    ConjuntosDisjuntos c(x * y);
    for (int i = 0; i < x; ++i) {
        for (int j = 0; j < y; ++j) {
            if (mapa[i][j] == 1) {
                comprobarAlrededorInicial(mapa, c, i, j, x, y);
            }
        }
    }
    return c;
}

//El coste de la funciÛn es O(x * y), siendo x las filas e y las columnas del mapa, ya que leer la matriz tiene coste x * y, unir las manchas
// (funciÛn unirInicial() ) tiene coste x * y, y por ultimo es coste lineal en m (numero de manchas nuevas), por lo que  el coste total
//est· incluido en O(x * y)
bool resuelve() {
    string line;
    int x, y;
    cin >> x;
    if (!cin) return false;
    
    cin >> y;
    cin.ignore();
    Matriz<size_t> field = Matriz<size_t>(x, y, 0);
    for (int i = 0; i < x; ++i) {
        getline(cin, line);
        for (int j = 0; j < y; ++j) {
            if (line[j] == '#') field[i][j] = 1;
        }
    }
    ConjuntosDisjuntos conj = unirInicial(x, y, field);
    cout << conj.maximo() << endl;
    return true;
}

int main() {
    // ajustes para que cin extraiga directamente de un fichero
/*#ifndef DOMJUDGE
    std::ifstream in("casos.txt");
    auto cinbuf = std::cin.rdbuf(in.rdbuf());
#endif*/
    while (resuelve()) {}
    
    // para dejar todo como estaba al principio
/*#ifndef DOMJUDGE
    std::cin.rdbuf(cinbuf);
    system("PAUSE");
#endif*/
    return 0;
}