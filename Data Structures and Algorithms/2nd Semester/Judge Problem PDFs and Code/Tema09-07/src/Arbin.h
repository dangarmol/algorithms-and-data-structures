...
public:
	...
	*/
	...
	void numNodos(int &nodos, int &hojas, int &altura)
	{
		numNodos(_ra, nodos, hojas, altura);
	}
...
private:
	...
	static void numNodos(Nodo *p, int &nodos, int &hojas, int &altura)
	{
		if (p != NULL)
		{
			altura++;
			...
			numNodos(p->_iz, nodos, hojas, alturaIz);
			numNodos(p->_dr, nodos, hojas, alturaDr);

			if(altura == alturaIz && altura == alturaDr)
			{
				hojas++;
			}
			else
