...
public:
	...
	*/
	...
	void colar(Queue<T> &amigos, T const &nColar)
	{
		Nodo *pringao = _prim;

		while (pringao != NULL && nColar != pringao->_elem)
			pringao = pringao->_sig;

		if (pringao != NULL)
		{
			if (pringao->_sig == NULL)
				_ult = amigos._ult;

			amigos._ult->_sig = pringao->_sig;
			pringao->_sig = amigos._prim;
			amigos._prim = NULL; // como pase el destructor jode la lista de amigos y luego pasa el de la fila y entra en un nulo y se nos cuaja la coyunta
		}
	}
